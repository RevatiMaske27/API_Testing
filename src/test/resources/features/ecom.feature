Feature: ReqRes API Testing
  ...
  Scenario: Login with missing password
    ...
    When a POST request is sent to "/api/login" with only email 
    Then the response status should be 400
    And an error message should be returned

  Scenario: Register with valid data
    Given valid email and password
    When a POST request is sent to "/api/register"
    Then the response status should be 200
    And a token should be returned

  Scenario: Register with missing password
    Given only email is provided
    When a POST request is sent to "/api/register" with email field
    Then the response status should be 400
    And an error message should be returned
  ...
  Scenario: Update user with valid data
    ...
    When a PUT request is sent to "/api/users/2"
    Then the response status should be 200
    And updated data should be returned with name "neo"


  Scenario: Update user with invalid ID
    When a PUT request is sent to "/api/users/999" with name and job
    ...
    When a PATCH request is sent to "/api/users/2" with partial data
    Then the response status should be 200
